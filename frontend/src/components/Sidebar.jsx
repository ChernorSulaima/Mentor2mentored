    ...
    return (
        <div className="flex min-h-full flex-col items-start bg-base-100 is-drawer-close:w-14 is-drawer-open:w-64 pt-3">
            ...
            <ul className="menu w-full grow space-y-3 pt-10">
                ...
                {navItems.map((link) => (
                    <li
                        key={link.label}
                    >
                        <NavLink to={link.path} end={link.path === "/"} className={({ isActive }) => `${isActive ? "is-drawer-open:bg-base-200 text-info" : ""} is-drawer-close:tooltip is-drawer-close:tooltip-right flex items-center gap-3`} data-tip={link.label}>
            ...
            <div className="dropdown dropdown-top dropdown-hover border border-base-300 bg-base-200 w-full">
                <div tabIndex={0} role="button" className="flex items-center gap-3 pl-1 m-1">
